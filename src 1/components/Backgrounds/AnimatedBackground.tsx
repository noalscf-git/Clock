// src/components/Backgrounds/AnimatedBackground.tsx - исправленная версия
...
export const AnimatedBackground: React.FC<AnimatedBackgroundProps> = ({ settings, className }) => {
  ...
  useEffect(() => {
    ...
    // Получаем цвет фона из первого цвета или используем черный
    const getBackgroundColor = () => {
      return settings.color1 || '#000000';
    };

    // Инициализация частиц в зависимости от типа анимации
    const initParticles = () => {
      ...
      switch (settings.type) {
        ...
        case 'gradient':
          // Для градиента частицы не нужны
          particles = [];
          break;
      }
    };
    ...
    // Функции отрисовки для разных типов
    const drawParticles = () => {
      // Заливаем фон
      ...
      particles.forEach(p => {
        ...
        const color = p.color;
        ctx.fillStyle = `rgba(${parseInt(color.slice(1,3), 16)}, 
                              ${parseInt(color.slice(3,5), 16)}, 
                              ${parseInt(color.slice(5,7), 16)}, 
                              ${0.6 + Math.sin(time + p.x) * 0.2})`;
        ctx.fill();
    ...
    // Анимационный цикл с фиксированным FPS
    const animate = () => {
  ...
  return (
    <canvas
      ref={canvasRef}
      className={`${styles.animatedBg} ${className || ''}`}
      style={{
        position: 'fixed',
        top: 0,
        left: 0,
