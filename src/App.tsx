...
function App() {
  ...
  const [currentBackground, setCurrentBackground] = useLocalStorage('currentBackground', 'gradient1');
  const [folderImages, setFolderImages] = useLocalStorage<FolderImage[]>('folderImages', []);
  ...
  useEffect(() => {
    ...
    if (savedImages) {
      try {
        ...
        setFolderImages(images.map((data: string, index: number) => ({
          id: index,
          data: data,
